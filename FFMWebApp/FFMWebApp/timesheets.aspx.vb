...
Partial Class timesheets
    ...
    Sub page_load()
        ...
        Dim CausedPostback As String = Page.Request.Params.Get("__EVENTTARGET")

        If CausedPostback = "ctl00$CPH_MainContent$DDL_Type" Or CausedPostback = "ctl00$CPH_MainContent$DDL_Offices" Then
            DDL_ShowAsPM.SelectedValue = 0
            Session("ShowasPM") = "0"
        End If

        If CausedPostback = "ctl00$CPH_MainContent$DDL_ShowAsPM" Then
            DDL_Type.SelectedValue = 0
            Session("SelectedType") = "0"
            DDL_Offices.SelectedValue = 0
            Session("SelectedOffice") = "0"
        End If
    ...
    Function GenerateEmployeeWeekTotals(ByVal StartDate As DateTime, ByVal EndDate As DateTime, ByVal PMLoggedIn As String) As String
        ...
        If TypeFilter <> "0" Or PMFilter = "" Then
        ...
        If strEmployeeList.Length > 0 Then
            ...
            strResult.Append(MakeTimeListing(strEmployeeList, PMLoggedIn, StartDate, EndDate))
        Else
            strResult.Append("<tr><td colspan=""10"">No users matching the criteria</td></tr>")
        End If

        '----------------

        strResult.Append("<tr><td colspan=""10"" class=""pagination"">Page:&nbsp;&nbsp;")
        ...
        While PageCounter < (TotalEmployees / 20)
            ...
            If PageCounter = Session("ShowPage") Then
                strResult.Append(" <span class=""current"">" & PageCounter & "</span>&nbsp;&nbsp;")
            Else
                strResult.Append(" <a href=""timesheets.aspx?page=" & PageCounter & """>" & PageCounter & "</a>&nbsp;&nbsp;")
            End If
        ...
        If TypeFilter = "0" And PMFilter <> "" Then
            strResult.Append("<tr><td colspan=""10"" style=""border-top:1px dashed #3F317A""><strong>Users that have not entered any time this week:</strong><br>" & EmployeesWithNoTime(StartDate, EndDate) & "</td></tr>")
        End If
    ...
    Function GenerateProjectWeekTotals(ByVal StartDate As Date, ByVal enddate As Date) As String
        ...
        strResult.Append("<table class=""listing"" style=""width:200px;"" id=""ListEmployees"">")
        Dim strEmployeeList As String = ""
        strResult.Append("<tr><th>Job Number</th><th>Total Time</th></tr>")
        ...
        Try
            ...
            While drEmployees.Read
                ...
                strResult.Append("<tr><td><a href=""jobreport.aspx?startdate=" & StartDate & "&enddate=" & enddate & "&jobnumber=" & JobNumber & """>" & JobNumber & "</a></td><td>" & TotalJobTime & "</td></tr>")
